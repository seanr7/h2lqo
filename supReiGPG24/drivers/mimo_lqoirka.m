function [Er, Ar, Br, Cr, Mr, info] = mimo_lqoirka(E, A, B, C, M, r, opts)
% MIMO_LQOIRKA Iterative Rational Krylov Algorithm (IRKA) for model-order 
...
%       W(:, k) = ((mirroredPoles(k))*E.' - A.')\(2*(qoLeftTangents(k, 1)*[M1*V(:, 1), ..., Mp*V(:, 1)] + ... 
%                   + (qoLeftTangents(k, r)*[M1*V(:, r), ..., Mp*V(:, r)]) + ...
...
%   M    - p x n x n array containing symmetric quadratic-output matrices
%          M1, M2, ..., Mp in (2)
...
%   |                 | (default eye(m, r))                               |
%   +-----------------+---------------------------------------------------+
%   | loLeftTangents  | initial p x r matrix of left tangent directions   | 
%   |                 | (default eye(p, r))                               |
%   +-----------------+---------------------------------------------------+
%   | qoLeftTangents  | initial p x r^2   matrix of qo left tangent       |
%   |                 | directions                                        |
%   |                 | (default eye(p, r^2))                             |
%   +-----------------+---------------------------------------------------+
%
...
%
% Copyright (c) 2027 Sean Reiter
% All rights reserved.
% License: BSD 2-Clause license (see COPYING)
%
% Virginia Tech, USA
% Last editied: 1/8/2026
%
